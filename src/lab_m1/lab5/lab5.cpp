...
void Lab5::OnInputUpdate(float deltaTime, int mods)
{
    ...
    if (window->KeyHold(GLFW_KEY_3)) {
        orthoSize += 2.0f * deltaTime;
        float aspectRatio = window->props.aspectRatio;
        projectionMatrix = glm::ortho(-orthoSize * aspectRatio, orthoSize * aspectRatio, -orthoSize, orthoSize, 0.01f, 200.0f);
    }
...
void Lab5::OnKeyPress(int key, int mods)
{
    ...
    if (key == GLFW_KEY_O) {
        ...
        projectionMatrix = glm::ortho(-orthoSize * aspectRatio, orthoSize * aspectRatio,
                                      -orthoSize, orthoSize, 0.01f, 200.0f);
    }
    if (key == GLFW_KEY_P) {
